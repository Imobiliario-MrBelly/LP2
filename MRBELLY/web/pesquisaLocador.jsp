    ...
    <body style="background-color: #f0f0f0;">
        ...
        <div class="sidenav px-4 d-inline">
            ...
            <div class="my-5">
                ...
                <div class="mb-4"><a href="pesquisaLocador" class="quickBold  ativo">LOCADORES</a></div>
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        <c:forEach items="${Locadores}" var="locador">
                            <tr>
                                <th scope="row"><c:out value="${locador.id}"/></th>
                                <td><c:out value="${locador.pessoa.getNome()}"/></td>
                                <td><c:out value="${locador.login.getEmail()}"/></td>
                                <td><c:out value="${locador.pessoa.getCpf()}"/></td>
                                <td><c:out value="${locador.endereco.getUf()}"/></td>
                                <td><a class="btn btn-sm btn-primary" href="ManterLocador?acao=prepararOperacao&operacao=Editar&id=<c:out value="${locador.id}"/>">Editar</a></td>
                                <td><a class="btn btn-sm btn-danger" href="ManterLocador?acao=prepararOperacao&operacao=Excluir&id=<c:out value="${locador.id}"/>">Excluir</a></td>
                            </tr>
                        </c:forEach>
                    </tbody>
                </table>
